...
- **单文件输出**：MDX 可直接编译为独立 `.html` 文件。不含 Mermaid 的页面完全自包含、离线可开；含 Mermaid 的页面默认在运行时从 CDN 加载 Mermaid（构建更快、产物更小），需要离线单文件时加 `--inline-mermaid` 把它内联回 HTML。无图片时基础产物通常较小；实际体积主要取决于文档是否使用 KaTeX，以及是否将本地图片内联进 HTML。
...
skill 使用本项目提供的 CLI（npm 包 `concept-atlas-dense-explain`，当前 `0.6.1`）：
